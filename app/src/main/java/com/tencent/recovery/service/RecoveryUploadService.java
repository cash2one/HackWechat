...
public class RecoveryUploadService extends IntentService {
    ...
    public static synchronized void startAlarm(Context context, String str) {
        synchronized (RecoveryUploadService.class) {
            if (!Util.ov(str)) {
                CommonOptions gi = OptionFactory.gi(context);
                ...
                RecoveryLog.i(TAG, "startAlarm %s lastUploadTime %s", str, Util.gt(j));
                ...
                if (service == null || currentTimeMillis - j > gi.zRG) {
                    if (service != null) {
                        service.cancel();
                    ...
                    try {
                        ((AlarmManager) context.getSystemService("alarm")).set(0, (gi.zRG + currentTimeMillis) - 2000, PendingIntent.getService(context, 1000001, intent, 134217728));
                    } catch (Throwable e) {
                        RecoveryLog.printErrStackTrace(TAG, e, "alarmManager.setAlarm", new Object[0]);
                    }
                    RecoveryLog.i(TAG, "startAlarm pendingIntent success: %d will report %s", Integer.valueOf(r2.hashCode()), Util.gt(j));
                } else {
    ...
    public static void startAlarmAfter(Context context, String str, long j) {
        if (!Util.ov(str)) {
            ...
            RecoveryLog.i(TAG, "startAlarmAfter pendingIntent:%d %s", Integer.valueOf(service.hashCode()), Util.gt(r2));
        }
    }
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (!tryToUploadData) {
            startAlarmAfter(this, stringExtra, OptionFactory.gi(this).zRG);
        }
        RecoveryLog.i(TAG, "RecoveryUploadReceiver.uploadDataResult %b", Boolean.valueOf(tryToUploadData));
        if (RecoveryLog.cDk() instanceof RecoveryFileLog) {
