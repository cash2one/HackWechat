...
public class RecoveryHandleService extends IntentService {
    private static volatile boolean flS;
    ...
    protected final void onHandleIntent(Intent intent) {
        ...
        if (recoveryData == null || recoveryData.processName == null) {
            RecoveryLog.i("Recovery.RecoveryHandleService", "recoveryData is null %b %d %s %s", Boolean.valueOf(booleanExtra), Integer.valueOf(gk), stringExtra, stringExtra2);
            return;
        }
        ...
        if (flS) {
            ...
            RecoveryLog.i("Recovery.RecoveryHandleService", "start to handle %s's exception uuid: %s threadId: %d", recoveryData.processName, recoveryData.ned, Long.valueOf(Thread.currentThread().getId()));
            ...
            if (recoveryHandleResult.zRC && !booleanExtra) {
                ...
                long currentTimeMillis = (System.currentTimeMillis() + gi.zRH) - 2000;
                ((AlarmManager) getSystemService("alarm")).set(0, currentTimeMillis, PendingIntent.getService(this, 1000002, intent3, 0));
                RecoveryLog.i("Recovery.RecoveryHandleService", "startAlarm pendingIntent success: %d will retry %s", Integer.valueOf(r2.hashCode()), Util.gt(currentTimeMillis));
            }
