...
public class TinkerSoLoader {
    public static boolean a(String str, ShareSecurityCheck shareSecurityCheck, Intent intent) {
        String str2 = (String) shareSecurityCheck.AlR.get("assets/so_meta.txt");
        if (str2 == null) {
            return true;
        }
        ...
        while (it.hasNext()) {
            ...
            if (ShareBsDiffPatchInfo.a(shareBsDiffPatchInfo)) {
                hashMap.put(shareBsDiffPatchInfo.path + "/" + shareBsDiffPatchInfo.name, shareBsDiffPatchInfo.fqR);
            } else {
