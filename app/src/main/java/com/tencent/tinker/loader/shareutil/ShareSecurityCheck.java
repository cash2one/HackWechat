...
public class ShareSecurityCheck {
    ...
    public final HashMap<String, String> AlR = new HashMap();
    private final HashMap<String, String> AlS = new HashMap();
    ...
    public ShareSecurityCheck(Context context) {
        ...
        if (AlQ == null) {
            ...
            try {
                String bQ = SharePatchFileUtil.bQ(context2.getPackageManager().getPackageInfo(context2.getPackageName(), 64).signatures[0].toByteArray());
                AlQ = bQ;
                if (bQ == null) {
                    throw new TinkerRuntimeException("get public key md5 is null");
                }
                SharePatchFileUtil.cy(null);
            } catch (Throwable e) {
    ...
    public final HashMap<String, String> cHf() {
        if (!this.AlS.isEmpty()) {
            return this.AlS;
        }
        String str = (String) this.AlR.get("assets/package_meta.txt");
        if (str == null) {
            return null;
        }
        for (String str2 : str.split("\n")) {
            if (!(str2 == null || str2.length() <= 0 || str2.startsWith("#"))) {
                ...
                if (split != null && split.length >= 2) {
                    this.AlS.put(split[0].trim(), split[1].trim());
                }
            }
        }
        return this.AlS;
                ...
                while (entries.hasMoreElements()) {
                    ...
                    if (jarEntry != null) {
                        ...
                        if (!name.startsWith("META-INF/") && name.endsWith("meta.txt")) {
                            this.AlR.put(name, SharePatchFileUtil.a(jarFile2, jarEntry));
    ...
    private static boolean a(File file, Certificate[] certificateArr) {
        if (certificateArr.length > 0) {
            ...
            while (length >= 0) {
                try {
                    if (AlQ.equals(SharePatchFileUtil.bQ(certificateArr[length].getEncoded()))) {
                        return true;
                    }
                    length--;
                } catch (Exception e) {
