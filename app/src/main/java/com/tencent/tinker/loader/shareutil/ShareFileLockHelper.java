...
public class ShareFileLockHelper implements Closeable {
    private final FileOutputStream Alr;
    private final FileLock Als;
...
*/
    ...
    private ShareFileLockHelper(File file) {
        this.Alr = new FileOutputStream(file);
        Throwable th = null;
        ...
        while (i < 3) {
            int i2 = i + 1;
            try {
                fileLock = this.Alr.getChannel().lock();
