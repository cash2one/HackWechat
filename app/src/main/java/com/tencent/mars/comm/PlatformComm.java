...
public class PlatformComm {
    ...
    private static af handler = null;
    ...
    public static class C2Java {
        ...
        public static int getStatisticsNetType() {
            ...
            try {
                int netType = an.getNetType(PlatformComm.context);
                if (netType == -1) {
                    return -1;
                }
                if (an.is2G(PlatformComm.context)) {
                    return 3;
                }
                if (an.is3G(PlatformComm.context)) {
                    return 4;
                }
                if (an.is4G(PlatformComm.context)) {
                    return 5;
                }
                if (an.isWifi(netType)) {
                    return 1;
                }
                if (an.isWap(netType)) {
                    return 2;
        ...
        public static SIMInfo getCurSIMInfo() {
            try {
                ...
                int iSPCode = an.getISPCode(PlatformComm.context);
                if (iSPCode == 0) {
                    return null;
                }
                ...
                sIMInfo.ispName = an.getISPName(PlatformComm.context);
                return sIMInfo;
            } catch (Throwable e) {
                x.printErrStackTrace(PlatformComm.TAG, e, "", new Object[0]);
                return null;
        ...
        public static boolean isNetworkConnected() {
            ...
            if (PlatformComm.context == null) {
                ...
                try {
                    z = an.isNetworkConnected(PlatformComm.context);
                } catch (Throwable e) {
                    x.e(PlatformComm.TAG, exception2String(e));
                    x.printErrStackTrace(PlatformComm.TAG, e, "", new Object[z]);
                    Assert.assertTrue(e.getClass().getSimpleName() + ":" + e.getStackTrace()[z] + ", " + e.getStackTrace()[1], z);
