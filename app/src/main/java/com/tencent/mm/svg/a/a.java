...
public final class a {
    private static String xCl = ".svg.code.drawable";
    protected static int xCm = 3;
    private static String xCn = "";
    private static Map<Integer, c> xCo = new ConcurrentHashMap();
    private static Map<Integer, Picture> xCp = new ConcurrentHashMap();
    private static Map<String, Picture> xCq = new ConcurrentHashMap();
    private static Float xCr;
    private TypedValue xCj = new TypedValue();
    private byte[] xCk = new byte[0];
    ...
    private static Drawable a(Resources resources, int i, float f) {
        ...
        if (i == 0) {
            ...
            if (f > 0.0f) {
                ...
                picture = (Picture) xCq.get(str4);
                if (picture == null) {
                    picture = a(f2, a(resources, i, str), f3);
                    xCq.put(str4, picture);
                }
                return new c(picture, i);
            }
            picture = (Picture) xCp.get(Integer.valueOf(i));
            if (picture == null) {
                picture = a(f2, a(resources, i, str), f3);
                xCp.put(Integer.valueOf(i), picture);
            }
            return new c(picture, i);
        }
    }
    ...
    public final boolean d(Resources resources, int i) {
        try {
            if (resources.getResourceTypeName(i).equals("raw")) {
                String charSequence;
                synchronized (this.xCk) {
                    TypedValue typedValue = this.xCj;
                    if (typedValue == null) {
    ...
    public static Map<Integer, c> clb() {
        return xCo;
    }
    ...
    private static Drawable c(Resources resources, int i, float f) {
        ...
        try {
            ...
            float f2 = resources.getDisplayMetrics().density;
            float f3 = (f > 0.0f ? f : 1.0f) * ((viewPort[0] / ((float) xCm)) * f2);
            float f4 = (viewPort[1] / ((float) xCm)) * f2;
            if (f <= 0.0f) {
                f = 1.0f;
            }
            float f5 = f4 * f;
    ...
    private static c a(Resources resources, int i, String str) {
        Object obj = "";
        try {
            if (xCo.containsKey(Integer.valueOf(i))) {
                return (c) xCo.get(Integer.valueOf(i));
            }
            ...
            xCo.put(Integer.valueOf(i), cVar);
            return cVar;
        } catch (Throwable e) {
            x.printErrStackTrace("MicroMsg.SVGCompat", e, "", new Object[0]);
    ...
    private static Picture a(float f, c cVar, float f2) {
        ...
        if (cVar == null) {
            ...
            if (xCr == null) {
                xCr = Float.valueOf(f / ((float) xCm));
            }
            float floatValue = xCr.floatValue() * f2;
