package com.tencent.mm.plugin.game.c;

import com.tencent.mm.protocal.c.bcv;
import com.tencent.mm.protocal.c.fd;
import e.a.a.b;
import e.a.a.c.a;
import java.util.LinkedList;

public final class dx extends bcv {
    public String hvC;
    public boolean nfA;
    public String nfl;
    public String nfn;
    public String nhF;

    protected final int a(int i, Object... objArr) {
        if (i == 0) {
            a aVar = (a) objArr[0];
            if (this.nhF == null) {
                throw new b("Not all required fields were included: Lang");
            } else if (this.nfn == null) {
                throw new b("Not all required fields were included: AppID");
            } else {
                if (this.wIV != null) {
                    aVar.fW(1, this.wIV.bke());
                    this.wIV.a(aVar);
                }
                if (this.nhF != null) {
                    aVar.g(2, this.nhF);
                }
                if (this.nfn != null) {
                    aVar.g(3, this.nfn);
                }
                if (this.nfl != null) {
                    aVar.g(4, this.nfl);
                }
                if (this.hvC != null) {
                    aVar.g(5, this.hvC);
                }
                aVar.al(6, this.nfA);
                return 0;
            }
        } else if (i == 1) {
            if (this.wIV != null) {
                r0 = e.a.a.a.fT(1, this.wIV.bke()) + 0;
            } else {
                r0 = 0;
            }
            if (this.nhF != null) {
                r0 += e.a.a.b.b.a.h(2, this.nhF);
            }
            if (this.nfn != null) {
                r0 += e.a.a.b.b.a.h(3, this.nfn);
            }
            if (this.nfl != null) {
                r0 += e.a.a.b.b.a.h(4, this.nfl);
            }
            if (this.hvC != null) {
                r0 += e.a.a.b.b.a.h(5, this.hvC);
            }
            return r0 + (e.a.a.b.b.a.dX(6) + 1);
        } else if (i == 2) {
            e.a.a.a.a aVar2 = new e.a.a.a.a((byte[]) objArr[0], unknownTagHandler);
            for (r0 = bcv.a(aVar2); r0 > 0; r0 = bcv.a(aVar2)) {
                if (!super.a(aVar2, this, r0)) {
                    aVar2.cJE();
                }
            }
            if (this.nhF == null) {
                throw new b("Not all required fields were included: Lang");
            } else if (this.nfn != null) {
                return 0;
            } else {
                throw new b("Not all required fields were included: AppID");
            }
        } else if (i != 3) {
            return -1;
        } else {
            e.a.a.a.a aVar3 = (e.a.a.a.a) objArr[0];
            dx dxVar = (dx) objArr[1];
            int intValue = ((Integer) objArr[2]).intValue();
            switch (intValue) {
                case 1:
                    LinkedList Jl = aVar3.Jl(intValue);
                    int size = Jl.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        byte[] bArr = (byte[]) Jl.get(intValue);
                        com.tencent.mm.bq.a fdVar = new fd();
                        e.a.a.a.a aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (boolean z = true; z; z = fdVar.a(aVar4, fdVar, bcv.a(aVar4))) {
                        }
                        dxVar.wIV = fdVar;
                    }
                    return 0;
                case 2:
                    dxVar.nhF = aVar3.Avy.readString();
                    return 0;
                case 3:
                    dxVar.nfn = aVar3.Avy.readString();
                    return 0;
                case 4:
                    dxVar.nfl = aVar3.Avy.readString();
                    return 0;
                case 5:
                    dxVar.hvC = aVar3.Avy.readString();
                    return 0;
                case 6:
                    dxVar.nfA = aVar3.cJC();
                    return 0;
                default:
                    return -1;
            }
        }
    }
}
