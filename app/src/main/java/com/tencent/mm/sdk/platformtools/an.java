...
public final class an {
    private static int nEb = -1;
    private static int nowStrength = 10000;
    private static PhoneStateListener xgN = null;
    private static int xgO = 10000;
    ...
    public static synchronized void eT(Context context) {
        synchronized (an.class) {
            TelephonyManager telephonyManager = (TelephonyManager) context.getSystemService("phone");
            nEb = telephonyManager.getPhoneType();
            if (xgN == null) {
                xgN = new PhoneStateListener() {
                    public final void onSignalStrengthsChanged(SignalStrength signalStrength) {
                        super.onSignalStrengthsChanged(signalStrength);
                        if (an.nEb == 2) {
                            an.nowStrength = signalStrength.getCdmaDbm();
                        }
                        if (an.nEb == 1) {
                            an.nowStrength = (signalStrength.getGsmSignalStrength() * 2) - 113;
                        }
                        x.i("MicroMsg.NetStatusUtil", "PhoneStateListener  type:%d  strength:%d", Integer.valueOf(an.nEb), Integer.valueOf(an.nowStrength));
                    }
                };
                telephonyManager.listen(xgN, 256);
            }
        }
    }
    ...
    public static synchronized List<a> eU(Context context) {
        List<a> list;
        int lac;
                    ...
                    if (telephonyManager.getPhoneType() == 2) {
                        try {
                            ...
                            if (cdmaCellLocation != null) {
                                str3 = nowStrength == xgO ? "" : nowStrength;
                        ...
                        try {
                            ...
                            if (gsmCellLocation != null) {
                                ...
                                lac = gsmCellLocation.getLac();
                                if (!(lac >= 65535 || lac == -1 || cid == -1)) {
                                    linkedList.add(new a(str, networkOperator, String.valueOf(lac), String.valueOf(cid), nowStrength == xgO ? "" : nowStrength, "gsm", "", "", ""));
                                }
                            }
                        } catch (Throwable e22) {
                            x.printErrStackTrace("MicroMsg.NetStatusUtil", e22, "", new Object[0]);
