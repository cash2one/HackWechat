...
public class FreewifiActivateWeChatNoAuthStateUI extends FreeWifiActivateNoAuthStateUI {
    ...
    public final void a(State state) {
        ...
        if (state == State.CONNECTED && d.AJ(this.ssid) && this.mGt) {
            ...
            this.mIM = d.aLD();
            this.ftq = d.aLC();
            x.i("MicroMsg.FreeWifi.FreewifiActivateWeChatNoAuthStateUI", "now has connect the ap, check from server rssi is :  %d, mac : %s, ssid is : %s", new Object[]{Integer.valueOf(this.ftq), this.mIL, this.mIM});
    ...
    protected final int aMt() {
        ...
        x.i("MicroMsg.FreeWifi.FreewifiActivateWeChatNoAuthStateUI", "get connect state = %d", new Object[]{Integer.valueOf(AI)});
        if (AI == 0) {
            return -2014;
        }
        return AI;
